...
class Common_Service_Menu {
	...
	private function getLevelsByUid($groupInfo) {
		if ($groupInfo['groupid'] == 0) {
			...
			list($this->usersite, $this->userlevels) = $this->formartUserLevels($groupInfo['rvalue']);
	...
	private function filterMenu(&$usermenu, $isroot) {
		foreach ($usermenu as $key => $value) {
			if ($isroot && !in_array($value['id'], $this->usersite)) {
				unset($usermenu[$key]);
			}
	...
	private function getSiteMenu($site) {
		$file = $this->sitefile;
		if (is_file($file)) {
			list ($config, $view, $this->extends, $this->noVerify) = require $file;
