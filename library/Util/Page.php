...
class Util_Page {
     ...
     * @param unknown_type $ajaxCallBack
     * @return string
     */
    static public function show_page_aimi($count, $page, $perPage, $url, $split='=', $ajaxCallBack = '',$class='') {
    	list($count, $page, $perPage) = array(intval($count), intval($page), intval($perPage));
    	(!$page || $page < 1) && $page = 1;
    	if ($perPage < 1 || $count <= $perPage) return '';
    	$maxPage = ceil($count / $perPage);
    	...
    	$pages = '<div class="pagination '.$class.'">';
    	$preArrow = $nextArrow = $firstPage = $nextArrow = '';
      if ($maxPage > 7) {
    		list($pre, $next) = array($page - 1, $page + 1);
    		$page > 1 && $preArrow = "<a class=\"page-star\" href=\"{$url}page{$split}{$pre}$mao\"{$ajaxurl}>&#x4E0A;&#x4E00;&#x9875;</a>";
    		$page < $maxPage && $nextArrow = "<a class=\"page-star\" href=\"{$url}page{$split}{$next}$mao\"{$ajaxurl}>&#x4E0B;&#x4E00;&#x9875;</a>";
    	}   
	    if ( $page != 1 ){
	         $first = "<a href=\"{$url}page{$split}1$mao\"{$ajaxurl} class=\"page-star\">首页</a>;";
	     }else{
		      $first  ='<a href="javascript:void(0)" class="page-star">首页</a>';
	     }
	     if ( $page < $maxPage ){
	      	$finish = "<a href=\"{$url}page{$split}{$maxPage}.$mao\"{$ajaxurl} class=\"page-end\">末页</a>;";
	     }else{
	      	$finish  ='<a href="javascript:void(0)" class="page-end">末页</a>';
	      } 
    	$page != 1 && $firstPage = "<a href=\"{$url}page{$split}1$mao\"{$ajaxurl}>" . (($maxPage > 7 && $page - 3 > 1) ? '1</a><span class="page-break">....</span>' : '1</a>');
    	$page != $maxPage && $lastPage = (($maxPage > 7 && $page + 3 < $maxPage) ? "<span class=\"page-break\">...</span><a href=\"{$url}page{$split}{$maxPage}$mao\"{$ajaxurl}>$maxPage</a>" : "<a href=\"{$url}page{$split}{$maxPage}$mao\"{$ajaxurl}>$maxPage</a>");
    	...
    	//$tmpPages .= "<b>$page</b>";
    	$tmpPages .= "<span class=\"page-cur\">$page</span>";
    	$nextFlag = 4 - $preFlag + (!$firstPage ? 1 : 0);
    	if ($page < $maxPage) {
    		for ($i = $page + 1; $i < $maxPage && $i <= $page + $nextFlag; $i++) {
    	$tmpPages .= "<a href=\"{$url}page{$split}$i$mao\"{$ajaxurl}>$i</a>";
    	...
    	$pages .= $first.$preArrow . $firstPage . $tmpPages . $lastPage . $nextArrow.$finish;
    	//$jsString = "var page=(value>$maxPage) ? $maxPage : value; " . ($ajaxurl ? "$ajaxCallBack('{$url}page{$split}'+page);" : " location='{$url}page{$split}'+page+'{$mao}';") . " return false;";
    	//$maxPage > 7 && $pages .= "<div class=\"fl\">&#x5230;&#x7B2C;</div><input type=\"text\" size=\"3\" onkeydown=\"javascript: if(event.keyCode==13){var value = parseInt(this.value); $jsString}\"><div class=\"fl\">&#x9875;</div><button onclick=\"javascript:var value = parseInt(this.previousSibling.previousSibling.value); $jsString\">&#x786E;&#x8BA4;</button>";
    	$pages .= '</div>';
    	return $pages;
